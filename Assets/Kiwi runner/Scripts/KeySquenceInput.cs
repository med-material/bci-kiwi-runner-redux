...
public class KeySquenceInput : MonoBehaviour
{
    ...
    [SerializeField]
    private float sequenceTimeLimit_ms = 0.5f; // the longest time that the sequence may take (500 ms time limit)
    [SerializeField]
    private float deadzoneTimeLimit_ms = 0.5f; // the time it takes before we consider an input to belong to a new sequence.
